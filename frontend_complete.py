...
def init_websocket():
    if 'websocket' not in st.session_state:
        try:
            ...
            # ws_uri = f"wss://{your_domain}/ws"
            
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            ws = loop.run_until_complete(websockets.connect(ws_uri))
            ...
            def start_listener():
                ...
                while True:
                    try:
                        message = loop.run_until_complete(ws.recv())
...
def handle_ws_message(data):
    if data['type'] == 'analysis_update':
        st.session_state.last_ai_result = data['data']
        st.session_state.last_update = data['timestamp']
        st.experimental_rerun()
    elif data['type'] == 'frame_update':
        st.session_state.latest_frame = data['data']
